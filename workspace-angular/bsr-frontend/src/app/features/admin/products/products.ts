...
export class AdminProducts implements OnDestroy {
  ...
  submit(): void {
    ...
    if (this.hasDuplicateProductName(normalizedName, selectedProductId)) {
      this.toastService.showError('Ya existe un producto con ese nombre. Cambialo antes de guardar.');
      return;
    }
    ...
    request$.pipe(finalize(() => this.saving.set(false))).subscribe({
      ...
      error: (error) => {
        this.toastService.showError(
          this.translateProductSaveError(error.error?.message),
        );
      },
    });
  }
  ...
  private generateUniqueSku(categoryId: number | null, excludedProductId: number | null = null): string {
  ...
  private hasDuplicateProductName(name: string, excludedProductId: number | null = null): boolean {
    ...
    return this.products().some(
      (product) =>
        ...
        this.normalizeProductName(product.name) === name,
    );
  }
  ...
  private translateProductSaveError(message: string | null | undefined): string {
    ...
    switch (normalizedMessage) {
      ...
      case 'Only image files are allowed':
        return 'El archivo seleccionado debe ser una imagen valida.';
